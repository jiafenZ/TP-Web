...
<script>
import { projectList, addMysql, mysqlList, updateMysql, deleteMysql } from '@/api/project'
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        host: [
          ...
          { max: 30, message: '连接地址长度不能超过30位' }
        ],
        port: [
          { required: true, message: '端口号必填', trigger: 'blur' },
          { max: 10, message: '端口号长度不能超过30位' }
        ],
        user: [
  ...
  methods: {
    ...
    getList() {
      ...
      mysqlList(this.listQuery).then((response) => {
